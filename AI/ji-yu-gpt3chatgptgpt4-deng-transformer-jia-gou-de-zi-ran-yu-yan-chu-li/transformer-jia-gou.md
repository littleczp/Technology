...
每层以这两个子层为主：一个多头注意力子层和一个前馈神经网络子层。

每层先从前一层学习，进行点积操作，然后从不同角度探索序列中词元的相关性。

***

### 输入嵌入

输入嵌入子层将输入词元转换为维度 d<sub>model</sub>=512 的特征向量。工作方式为：通过词元分析器（tokenizer）将句子拆分为词元。
...
Transformer 需要知道单词在序列中的位置，所以通过单位球面求正弦值（偶数）和余弦值（奇数）来表示位置编码。
