...
* 知识的局限性：模型自身的知识取决于它的训练数据，一些实时性的、非公开的或离线的数据是无法获取到的。
* 幻觉问题：所有的AI模型的底层原理都是基于数学概率，其模型输出实质上是一系列数值运算，当它不知道答案时，为了让句子通顺会编造一定的事实。
* 数据安全性：没有企业愿意承担数据泄露的风险，将私域数据上传第三方平台进行训练。
* 成本效益：学会新知识的另一种方法是"微调"（Fine-tuning），但这很难维护，选择外挂知识库的成本会较低一些。
...
> 通过规则和标签进行"前置剪枝"，缩小数据集。

* 在数据准备阶段，为切块打上标签（如：时间=2026年、部门=研发、文档类型=技术）。
* 自查询：利用大模型将 Query 拆解为一个结构化请求（如：`Filter: year > 2025` + `Query`），这样数据库可以先过滤掉不符合条件的向量，在更小的子集里做向量检索
...
1. 数据质量：需要解决复杂的文档格式、重复内容、过期内容、表情等。
2. 知识时效性：检索时同时召回新旧版本导致的错误答案。

> 数据清洗、Chunk 策略、版本控制、增量索引、知识过期机制
{% endstep %}

{% step %}
### 检索效果

用户 Query 和知识库中的原始文本可能存在**语义表达差异**，单纯依赖向量检索不一定能够找到真正相关的信息。

> 混合检索、父子切块、Query 变形与增强、元数据过滤、多阶段检索、重排

关注召回覆盖率、噪声比例以及排序质量等指标。
{% endstep %}
...
<summary>搜索系统与 RAG 的区别?</summary>

搜索解决的是找到相关信息，RAG在找到相关信息的基础上基于这些相关信息进行回答。

传统搜索一般会采用倒排索引、BM25 等技术；现代搜索也会加入 Embedding 和向量检索，形成混合检索。

RAG 在检索后会对知识进行筛选、排序和整理，将有价值的信息合并到大模型的上下文中，再通过提示词润色答案。生成后也可以增加一道校验来保证回答有所依据。
