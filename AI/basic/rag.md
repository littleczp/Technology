...
最难：不是把 RAG Pipeline 搭出来，而是建立一套能够**持续优化**的闭环。线上问题最终很难简单归因于“模型不好”或者“Embedding 不好”。需要通过 Trace（可观测性） 和 Evaluation（可量化的指标和测试集） 把问题拆解成 数据处理 → 检索 → 生成 → 端到端 几个阶段。通过分层评估和线上反馈闭环，持续优化 RAG。
